...
namespace cApp
{
    public class clsSpar
    {
        ...
        public clsSpar CopyZtoPS(int[,] Z)
        {
            ...
            P.n = Z.GetLength(0);  //filas
            P.m = Z.GetLength(1);  //columnas
            ...
            // recoremos las filas
            for(int i = 0; i < n; i++)
            {     // recoremos las columas
                for (int j = 0; j < m; j++)
                {
                    if (Z[i, j] > 0)
                    {
                        P.PS[P.Cant, 0] = j;  //columas
                        P.PS[P.Cant, 1] = i;  //filas
                        P.PS[P.Cant, 2] = Z[i, j];
                        P.Cant++;
                    }
                }
        ...
        public clsPila DiagonalSegunda(int[,] PS)
        {
            ...
            int cantidad = PS[0, 2]; //12
            int columna = PS[0, 0]; //7
